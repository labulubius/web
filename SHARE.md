...
Share metadata is stored in individual JSON files on `web`; there is no Supabase migration. The administrator can create nested folders and upload files into them. Each folder has its own unguessable `/s/<UUID>` public link showing its direct files and child folders; there is no public root directory or folder search. Existing files without a `folderId` remain at the administrator's root and keep their `/f/<UUID>` links. Folder links are bearer links: sharing a parent grants access to its descendants; sharing only a child does not reveal its parent. Deleting a folder recursively revokes the folder, descendants and their file links at the origin. A file download also checks the complete folder ancestry to fail closed if cleanup is interrupted.

Back up the entire `SHARE_DATA_DIR` including the new `folders/` directory along with `blobs/`, `thumbs/` and `meta/`. On deletion metadata is removed before the blob; a failed unlink can leave orphan blobs. Review backup size and orphan files periodically. To move the storage, copy the entire directory and update `SHARE_DATA_DIR`.
